    ...
    return (
        <main className="flex min-h-screen flex-col items-center">
            ...
            <div id="form" className="p-3 ">
                <p className={`text-5xl bg-gradient-to-b from-[#ffffff] to-[#2e2e2e] inline-block text-transparent bg-clip-text leading-[4rem] font-bold ${notoSans.className}`}>Introducing Elecbits Flux</p>
                ...
                <div className="w-full flex flex-col items-center ">
                    <form className="w-10/12 mt-8 grid grid-cols-2 gap-4 mb-10" onSubmit={handleSubmit}>
                        <div className="col-span-2">
                            ...
                            <input placeholder="Enter Your Mail Id" className="w-full px-4 py-2 bg-gray-800 text-gray-400 border-b-2 border-gray-700 rounded focus:outline-none focus:border-gray-500" name="email" type="email" value={formData.email} onChange={handleChange} required />
                        </div>
                        <div className="col-span-1">
                            <label>{"Name"}</label>
                            <div className="mt-2"></div>
                            <input placeholder="Enter Your Name" className="w-full px-4 py-2 bg-gray-800 text-gray-400 border-b-2 border-gray-700 rounded focus:outline-none focus:border-gray-500" name="name" type="text" value={formData.name} onChange={handleChange} required />
                        </div>
                        <div className="col-span-1">
                            <label>{"Phone Number"}</label>
                            <div className="mt-2"></div>
                            <input placeholder="Enter Your Phone Number" className="w-full px-4 py-2 bg-gray-800 text-gray-400 border-b-2 border-gray-700 rounded focus:outline-none focus:border-gray-500" name="phoneNumber" type="text" value={formData.phoneNumber} onChange={handleChange} required />
                            {phoneError && <p className="text-red-500 text-xs mt-1">{phoneError}</p>}
                        </div>
                        <div className="col-span-1">
                            <label>{"Organization Name"}</label>
                            <div className="mt-2"></div>
                            <input placeholder="Enter Organization Name" className="w-full px-4 py-2 bg-gray-800 text-gray-400 border-b-2 border-gray-700 rounded focus:outline-none focus:border-gray-500" name="organizationName" type="text" value={formData.organizationName} onChange={handleChange} required />
                        </div>
                        <div className="col-span-1">
                            <label>{"Designation"}</label>
                            <div className="mt-2"></div>
                            <input placeholder="Enter Designation" className="w-full px-4 py-2 bg-gray-800 text-gray-400 border-b-2 border-gray-700 rounded focus:outline-none focus:border-gray-500" name="designation" type="text" value={formData.designation} onChange={handleChange} required />
                        </div>
                        <button type="submit" className={`w-full col-span-2 mt-4 bg-[#232331] hover:bg-[#3365fb] py-4 rounded-md ${buttonStatus ? 'cursor-not-allowed' : ''}`} disabled={buttonStatus}>
                            {buttonStatus ? (
                                ...
                                <>
                                    Join the Waitlist for Priority Access <Image className="inline-block" src={"/RightArrow.png"} alt="" width={20} height={20} />
                                </>
                            )}
                        </button>
                    </form>
